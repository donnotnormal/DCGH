...
from model.hash_model import DCGH as DCGH
import os
...
from model.hash_model import TwoLayerHashNet
...
import time
#from utils.feature_train import Cgh
#import math
...
class Trainer(TrainBase):
    ...
    def _init_model(self):
        ...
        #hash_net_optimizer = torch.optim.Adam(hash_net.parameters(), lr=0.001)
        #self.cgh = Cgh().to(1)
        #self.hash_net = TwoLayerHashNet(input_dim=self.args.numclass, hidden_dim=128, output_dim=self.args.output_dim).to(self.rank)
        self.hyp = HyP().to(1)
        self.optimizer_loss = torch.optim.SGD(params=self.hyp.parameters(), lr=0.02, momentum=0.9, weight_decay=0.0005)
        #self.optimizer_loss1 = torch.optim.SGD(params=self.cgh.parameters(), lr=0.02, momentum=0.9, weight_decay=0.0005)
        #self.hash_net_optimizer = torch.optim.Adam(self.hash_net.parameters(), lr=0.01)
        #params=self.hyp.parameters()
        self.total_time = 0
        print(self.model)
    ...
    def _init_dataset(self):
        ...
        self.args.index_file = os.path.join("/home/cgh/DCGH-main/dataset", self.args.dataset, self.args.index_file)
        self.args.caption_file = os.path.join("/home/cgh/DCGH-main/dataset", self.args.dataset, self.args.caption_file)
        self.args.label_file = os.path.join("/home/cgh/DCGH-main/dataset", self.args.dataset, self.args.label_file)
    ...
    def train_epoch(self, epoch):
        ...
        #self.set_alpha(epoch)
        self.change_state(mode="train")
        self.logger.info(">>>>>> epochs: %d/%d"%(epoch, self.args.epochs))
        all_loss = 0
        # print("------------------", self.hyp.max_off_diag, "----------------------")
        #print(self.hyp.proxies)
        #print(self.hyp.M)
        for image, text, label, index in self.train_loader:
            ...
            index = index.numpy()
            # inputs = torch.eye(self.args.numclass).to(self.rank)
            #             # class_proxies = self.hash_net(inputs,self.alpha)

            #self.hyp.proxies.data = class_proxies
            ...
            #     # self.hash_net_optimizer.zero_grad()
            #     loss.backward()
            #     self.optimizer.step()
            #     self.optimizer_loss.step()
            #     # self.hash_net_optimizer.step()
            #     self.total_time += time.time() - start_time
            #     # 第一种损失函数
            # else:
            #     loss = self.hyp.B_loss(hash_img, hash_text, label)
            #     all_loss += loss
            #
            #     self.optimizer.zero_grad()
            #     # self.optimizer_loss.zero_grad()
            ...
            #     # self.hash_net_optimizer.step()
            #     self.total_time += time.time() - start_time
            #     # self.hyp.proxies.data = torch.tanh(self.hyp.proxies.data)
            #loss = self.hyp(hash_img, hash_text, label)
            # img_feature = self.model.clip_encode_image(image)
            # txt_feature = self.model.clip_encode_text(text)

            loss = self.hyp(hash_img,hash_text,label)#+self.hyp.B_loss(hash_img,hash_text,label)
            #+0.8*self.cgh(img_feature, txt_feature, label)self.hyp(hash_img, hash_text, label)+

            all_loss += loss

            self.optimizer.zero_grad()
            self.optimizer_loss.zero_grad()
            #self.optimizer_loss1.zero_grad()
            # self.hash_net_optimizer.zero_grad()
            loss.backward()
            self.optimizer.step()
            self.optimizer_loss.step()
            #self.optimizer_loss1.step()
            #self.hash_net_optimizer.step()
            self.total_time += time.time() - start_time
            #self.hyp.proxies.data = torch.tanh(self.hyp.proxies.data)
